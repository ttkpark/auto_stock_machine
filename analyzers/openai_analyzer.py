...
from utils.prompt_manager import build_buy_prompt, build_sell_prompt
    ...
    def __init__(self):
        ...
        if not api_key:
            raise EnvironmentError(
                ".env 파일에 OPENAI_API_KEY 가 설정되어 있지 않습니다."
            )
        self.client = OpenAI(api_key=api_key)
        self.model_name = os.environ.get("OPENAI_MODEL_NAME", "").strip() or DEFAULT_MODEL_NAME
